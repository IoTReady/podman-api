...
```sh
forgejo issue create IoTReadyNext/podman-api --title="..." --body="..."
forgejo issue view   IoTReadyNext/podman-api 42
forgejo issue close  IoTReadyNext/podman-api 42
forgejo pr create    IoTReadyNext/podman-api --title="..." --head=<branch> --base=main --body="..."
forgejo pr merge     IoTReadyNext/podman-api 17 --method=squash
forgejo <resource> --help    # detailed help
```

`OWNER/REPO` is always `IoTReadyNext/podman-api`. Add `--json` for machine-readable output.
...
```sh
git remote -v
# origin   ssh://git@git.iotready.com:2222/IoTReadyNext/podman-api.git  (private, source of truth)
...
forgejo pr create IoTReadyNext/podman-api --head=feat/my-change --base=main --body="..."
# ... review happens on Forgejo, then:
forgejo pr merge IoTReadyNext/podman-api <n> --method=squash
...
(`git.iotready.com/IoTReadyNext/podman-api-pro`) that imports this one as a Go dependency.
...
```
1. File issue on IoTReadyNext/podman-api describing the seam interface
...
Operator docs live in both the Forgejo wiki (`git.iotready.com/IoTReadyNext/podman-api/wiki`)
