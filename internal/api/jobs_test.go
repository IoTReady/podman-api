...
func newSrvWithJobs(t *testing.T, js store.JobStore) (*httptest.Server, string) {
	...
	srv := httptest.NewServer(NewRouter(svc, js, auth.NewKeyStore(keys), nil, nil, nil, "", nil))
	t.Cleanup(srv.Close)
	return srv, tok
}
...
func newSrvWithCancel(t *testing.T, js store.JobStore, c JobCanceller) (*httptest.Server, string) {
	...
	hosts := []config.Host{{ID: "h1", Addr: "unix", Socket: "/x"}}
	svc := instance.NewService(fake.New(), hosts)
	srv := httptest.NewServer(NewRouter(svc, js, auth.NewKeyStore(keys), nil, nil, c, "", nil))
	t.Cleanup(srv.Close)
	return srv, tok
}
